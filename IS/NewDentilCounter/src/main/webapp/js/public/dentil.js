...
selectedDay.addEventListener("click", function (e) {
  ...
  if (tableDay > 0 && tableDay < 32) {
	...
	loadElements(updateListOfAppointments, 
	GET_APPOINTMENTS_SAMEDAY_AND_DENTIST + getStringDate() + getStringIdDentist(), 
	GET_NUMBER_OF_APPOINTMENTS_SAMEDAY_AND_DENTIST + getStringDate() + getStringIdDentist());
  }
});
...
function updateListOfAppointments(){
	...
	for (let i=0;i<this.length;i++){
		...
		cell2.innerHTML = convertTime12To24(this[i].startTime);
...
function getDentist(){
	...
	$.ajax({
		url: "SpecialData?what=getDentists",
		type: "POST",
		data: null,
...
function getStringDate(){
	return "&date=" + currentDate.year + "-" + getMonthFromText(currentDate.month) + "-" + currentDate.day;
}

function getStringIdDentist(){
	return "&idDentist=" + document.getElementById("dentistID").value;
}
