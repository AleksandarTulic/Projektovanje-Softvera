...
public class AppointmentValidation {
	...
	private boolean checkIfAvailable(AppointmentDTO dto) {
		...
		List<AppointmentDTO> arrDTO = aService.selectSameDayAndDentist(dto.getStartDate(), dto.getIdDentist(), 0L, Long.MAX_VALUE);
		...
		for (AppointmentDTO i : arrDTO) {
			...
			if (ldt1.compareTo(ldt3) < 0 && ldt2.compareTo(ldt4) > 0)
				return false;
		}
	...
	private boolean checkWorkingTime(AppointmentDTO dto) {
		...
		DentistDTO dentistDTO = dService.selectWithDateWorkerAndDentist(dto.getStartDate() + "", dto.getIdDentist());
